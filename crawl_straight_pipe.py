...
def straight_pipe_crawl(use_propagation=True, sol="fmin_cg", freeze_nodes=0, num_steps=50, visualize=True):
    
    end_effector = ["N{}".format(x+1) for x in range(n_nodes)]
    rg = RobotStructGraph(num_nodes=n_nodes, task="straight_pipe_crawl", end_effector=end_effector)

    u1 = np.pi
    u2 = -np.pi/3
    u3 = np.pi/3

    x1, y1, z1 = base_coords(u1, -3.5)
    x2, y2, z2 = base_coords(u2, 0)
    x3, y3, z3 = base_coords(u3, 3.5)
    ...
    for iter in range(num_steps):
        ...
        z3 += .2
        rg.input_vn([x1, y1, z1], [x2, y2, z2], [x3, y3, z3])
        # robot_nodes = rg.update(base_lr, 500, target, tol=0.05)
        err, time, robot_nodes = rg.update(ff=robot_forward, fb=robot_backward, solver=sol, use_propagation=use_propagation)
        errs.append(err)
        times.append(time)
        configs.append(robot_nodes.copy())
    ...
    # visualization
    # scene set up
    scene = v.canvas(title='Straight Pipe Crawl', width=1280, height=720, up = v.vector(0,0,1),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_steps', type=int, default=50, help='Num of steps the robot crawl.')
    parser.add_argument('--visualize', type=bool, default=True, help='Do visualization.')
    args = parser.parse_args()

    errs, times = straight_pipe_crawl(use_propagation=args.use_propagation, sol=args.solver, freeze_nodes=args.freeze_num, num_steps=args.num_steps, visualize=args.visualize)
